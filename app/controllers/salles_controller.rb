class SallesController < ApplicationController
  ...
  def show
    ...
    @salle.serveurs.includes(:baie).includes(:modele => :category).joins(:baie).order('baies.ilot ASC, baies.position ASC, serveurs.position desc, serveurs.id desc').each do |s|
