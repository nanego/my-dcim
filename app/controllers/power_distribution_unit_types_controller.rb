...
class PowerDistributionUnitTypesController < ApplicationController
  ...
  def index
    authorize! @power_distribution_unit_types = PowerDistributionUnitType.joins(:manufacturer)
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_back_to_param_or power_distribution_unit_types_path, notice: t(".flashes.destroyed"), status: :see_other }
  ...
  def power_distribution_unit_type_params
    params.expect(
      power_distribution_unit_type: [
        :manufacturer_id, :name, :current_type, :documentation_url, :meter_global, :meter_per_socket, :meter_per_circuit,
        :socket_control, :socket_lock, :ip_snmp, :ip_modbus, :ip_ssh, :ip_webui, :rs485_modbus, :max_power_per_circuit,
        {
          circuits_attributes: [[
            :id, :_destroy, :name,
            { sockets_attributes: [%i[id name circuit_id port_type_id]] },
          ]],
        },
      ],
    )
