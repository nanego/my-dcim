class ModelesController < ApplicationController
  ...
  private
    ...
    def modele_params
      params.require(:modele).permit(:name, :color, :description, :published, :category_id, :architecture_id, :u, :marque_id, :nb_elts, enclosures_attributes: [:id, :modele_id, :_destroy, :position, :display, composants_attributes: [:type_composant_id, :enclosure_id, :name, :position, :_destroy, :id]])
    end
end
