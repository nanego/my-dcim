...
class ClustersController < ApplicationController
  ...
  def destroy
    if @cluster.destroy
      respond_to do |format|
        format.html { redirect_back_to_param_or clusters_url, notice: t(".flashes.destroyed") }
        format.json { head :no_content }
      end
    else
      respond_to do |format|
        format.html { redirect_back_to_param_or clusters_url, alert: @cluster.errors.full_messages_for(:base).join(", ") }
      end
    end
  end
