...
class DomainesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @domaine.save
        format.html { redirect_to_new_or_to(@domaine, notice: t(".flashes.created")) }
  ...
  def destroy
    if @domaine.destroy
      respond_to do |format|
        format.html { redirect_back_to_param_or domaines_url, notice: t(".flashes.destroyed") }
        format.json { head :no_content }
      end
    else
      respond_to do |format|
        format.html { redirect_back_to_param_or domaines_url, alert: @domaine.errors.full_messages_for(:base).join(", ") }
      end
    end
  end
