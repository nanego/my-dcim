...
RSpec.describe ServersProcessor do
  ...
  let(:attributes) do
    {
      frame: frames(:one), manager: managers(:one), domain: domains(:switch), modele: modeles(:one),
  ...
  describe "When searching on every fields" do # rubocop:disable RSpec/MultipleMemoizedHelpers
    ...
    let(:server) do
      Server.create!(name: "wood", numero: 1, **attributes, frame:, modele:, manager:, domain:, cluster:, stack:)
    end
