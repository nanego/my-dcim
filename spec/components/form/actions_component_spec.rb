...
RSpec.describe Form::ActionsComponent, type: :component do
  ...
  context "with new object" do
    ...
    it do # rubocop:disable RSpec/ExampleLength
      expect(rendered_component).to have_tag("div.col-12.sticky-bottom") do
        with_tag("a", text: "Annuler", with: { href: "/islets" })
        with_tag("a", text: "Annuler", count: 1)
        with_tag("input.btn-success", with: { type: "submit", value: "Créer" })
        without_tag("input.btn-info", with: { type: "submit", value: "Modifier" })
      end
    end
  end
  ...
  context "with persisted object" do
    ...
    it do # rubocop:disable RSpec/ExampleLength
      expect(rendered_component).to have_tag("div.col-12.sticky-bottom") do
        with_tag("a", text: "Annuler", with: { href: "/islets/1" })
        with_tag("a", text: "Annuler", count: 1)
        without_tag("input.btn-success", with: { type: "submit", value: "Créer" })
        with_tag("input.btn-info", with: { type: "submit", value: "Modifier" })
      end
    end
  end
end
